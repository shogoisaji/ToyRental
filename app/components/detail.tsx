...
export const Detail = ({ id }: { id: string }) => {
    ...
    return (
        <div className="flex flex-col justify-center items-center mt-10 md:flex-row md:items-start space-y-8 md:space-y-0 md:space-x-4 lg:space-x-8 max-w-6xl w-11/12 mx-auto">
            {toy ? (
                <>
                    <ProductImage
                        images={
                            toy!.images ? decodeImages()! : ['/no_image.png']
                        }
                    />
                    <div className="flex flex-col justify-between h-full w-full md:w-1/2 max-w-xs mx-auto space-y-2 min-h-128">
                        <BackToProductButton />
                        <ProductInfo
